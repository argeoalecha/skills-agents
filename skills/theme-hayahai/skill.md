...
|---|---|---|
| `assets/logos/hayah-ai-office-expert-logo.png` | Canonical arc + coral dot mark, `hayah-ai` wordmark, tagline "the office expert" baked into the raster. **Light-surface only** — arc and text are rendered in deep teal (`#0a3d3a`) with no light/cream variant, so on dark-teal or Midnight-theme backgrounds only the coral dot stays visible. Verified by compositing onto `#faf7f5` (legible) and `#0a3d3a` (arc/wordmark nearly invisible) | Project needs the parent Hayah-AI mark with a fixed tagline as a drop-in image on a light/cream surface (favicons, doc headers, light nav bars). For a dark surface, use the inline SVG's dark-background snippet instead — it has a cream-colored variant this raster doesn't |
| `assets/logos/levelup-logo-selection.png` | **LevelUp by hayah-ai** — a product sub-brand lockup sheet with a distinct bar-chart + coral-dot mark (not the parent arc device). Contains multiple ready variants: Midnight (dark teal bg, mint/white/coral) and Classic (cream bg, dark teal/coral) themes, each with Full lockup, Pill badge, Mark-only, and Dark-card layouts. Verified by cropping the Midnight-Full and Classic-Full lockups — both render with good contrast on their intended backgrounds | Building a "LevelUp" product surface, or any sub-brand that should visually nest under Hayah-AI while keeping its own icon/mark |

**How to offer these as choices:** when a project needs a logo, present the canonical arc+dot SVG (customizable tagline, theme-aware color) as the default, and mention these two raster options as alternates — the office-expert lockup for a fixed-tagline parent mark on light backgrounds, or the LevelUp sheet for a product sub-brand needing its own icon. Crop the specific variant (Midnight/Classic × Full/Pill/Mark/Dark-card) the project needs out of `levelup-logo-selection.png` rather than shipping the whole sheet. Never place `hayah-ai-office-expert-logo.png` on a dark/Midnight surface — it has no light-colored variant baked in.
