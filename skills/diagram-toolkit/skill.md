...
Top-level `direction`: `"TD"` (default) or `"LR"` for a linear pipeline/stage diagram that reads as a horizontal band. LR doesn't support `lanes`, skip-level bypass routing, or `loop` edges yet — combine them and the script raises rather than mis-drawing; use TD for anything with branching/convergence/retries. Per-node `fill`/`text_color` override the theme's node color for that one node (e.g. color-code pipeline stages by role rather than by shape); top-level `accent` overrides just the theme's accent color without switching the whole `--theme` — note that recolors every accent consumer, terminator node fills included, not only the eyebrow and edge/branch labels, so give the terminators their own `fill` if you want them off the accent. Top-level `eyebrow` renders a small label above the title.

If you're rendering the output of the `graph-topology-planner` skill as a client-facing or presentation deliverable (rather than the dev-facing Mermaid it produces by default), this is the renderer — it takes the same general-DAG shape as that skill's Nodes table, and it's the one that renders it as clean presentation output with swimlanes (`render_sketch.py` handles the same graph class but hand-drawn; `render_critical_path.py` is a DAG too but models a schedule, not a process): map its Topology clusters to `lanes`, Gates to a `decision` shape or a dedicated "Human" lane, and any Evaluator-Optimizer revise loop to `edge.loop: true`.
