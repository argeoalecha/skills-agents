...
find ~/.claude/skills -maxdepth 1 -mindepth 1 -type d -not -name "_backups" -not -name "synced" 2>/dev/null | sort > /tmp/_hk_all_dirs
...
| 2026-09-25 | 97 MB | 95 MB | ~1.45 MB — 126 items. AUTO-SAFE: 67 debug logs, 2 telemetry, 2 stale ide locks (11108/58294 — lock filenames are ports, not PIDs; checked the `pid` field inside each; 47858.lock left alone, VS Code PID 6546 live on `~/.claude`), 1 old shell snapshot (kept this session's), 9 file-history folders (952 KB, largest contributor). STALE (user approved): 38 session-env folders, 6 job folders (Sep 5–7, none pinned). ORPHANED: `diagram-toolkit/scripts/__pycache__/`. No .DS_Store, statsig, tasks, paste-cache, cache, stale plans, _backups, or agent-browser leftovers (single Chromium build 147.0.7727.57). New: `skills/synced/` (10 MB, harness-managed claude.ai skill sync — `anthropic-skills:*`) flagged by the no-skill.md scan and showing untracked in git; it is live, not orphaned — added to `.gitignore` and excluded from the orphan-dir scan. `diagram-toolkit/scripts/eval_render_flowchart.py` unmentioned in skill.md but is the committed eval suite (#6) — kept. Agent model IDs all current; 3 agents on `claude-opus-5` (Opus 5.5 available, optional upgrade). `secret-agent/` now lives at `agents/secret-agent/`. Total drop from 471 MB (Sep 6) is Claude Code's own `projects/` transcript cleanup, not this skill. |
